...
x_trainval, x_train_keras, x_test, x_test_keras, x_val, x_val_keras, y_trainval, y_test, y_val, features = \
...
if action == 0:
    ...
    callbacks_list = callbacks.ModelCheckpoint(tag + ' - Trained model.h5', monitor='val_acc', save_best_only=True,
                                               verbose=1, mode='max')
    history = model.fit(x_train_keras, y_trainval, batch_size=batch_size, callbacks=callbacks_list, epochs=epochs,
    ...
    utils.sweep_linear_models(x_trainval, y_trainval)

elif action == 2:
    utils.test_set_prediction(x_trainval, y_trainval, x_test, x_test_keras, y_test, x_val,
                              x_val_keras, y_val, model_to_load, batch_size)
elif action == 3:
    utils.linearmodels_coeffs_analysis(visualization, x_trainval, y_trainval, features)
